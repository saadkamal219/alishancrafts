...
app.get("/api/orders/export/csv", async (req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="alishan-crafts-orders-${date}.csv"`);
    res.send(csv);
  } catch (err) {
  ...
  try {
    ...
    const pdfBuffer = await new Promise((resolve, reject) => {
      ...
      doc.fontSize(7).fillColor(MUTED).font("Helvetica")
         .text(
           `Generated by Alishan Crafts Admin Panel  ·  ${new Date().toLocaleDateString("en-GB", { day:"2-digit", month:"long", year:"numeric" })}`,
...
app.post("/api/orders/export/sales-pdf", async (req, res) => {
  try {
    ...
    const totalPlatformFee  = records.length * PLATFORM_FEE;
    ...
    const pdfBuffer = await new Promise((resolve, reject) => {
      ...
      function drawFooter() {
        ...
        doc.font("Helvetica").fontSize(6.5).fillColor(C.dust)
           .text("Alishan Crafts  ·  Confidential — Authorised Personnel Only",
      ...
      function drawSubtotal(label, amount, count) {
        const pageFee = count * PLATFORM_FEE;
        ensureSpace(44);
        ...
        doc.font("Helvetica").fontSize(7).fillColor(C.dust)
           .text(`Platform Fee  (${count} order${count !== 1 ? "s" : ""} × Tk. ${PLATFORM_FEE})`, ML + 4, y + 3, { width: CW * 0.72 });
        doc.font("Helvetica-Bold").fontSize(8).fillColor("#c07a3a")
           .text(`Tk. ${pageFee.toLocaleString("en-IN")}`, ML, y + 2, { width: CW - 4, align: "right" });
        y += 18;
        rule(y, C.rule, 0.4);
        y += 10;
      }
      ...
      doc.font("Helvetica-Bold").fontSize(26).fillColor(C.white)
         .text("Alishan Crafts", ML, 34, { characterSpacing: 0.5 });
      ...
      doc.font("Helvetica").fontSize(8).fillColor(C.dust)
         .text("GRAND TOTAL (SALES)", ML, y, { characterSpacing: 1.2 });
      y += 14;
      doc.font("Helvetica-Bold").fontSize(28).fillColor(C.gold)
         .text(`Tk. ${grandTotal.toLocaleString("en-IN")}`, ML, y);
      y += 38;

      // Platform fee total on cover
      doc.font("Helvetica").fontSize(8).fillColor(C.dust)
         .text(`TOTAL PLATFORM FEE  (${records.length} orders × Tk. ${PLATFORM_FEE})`, ML, y, { characterSpacing: 0.8 });
      y += 12;
      doc.font("Helvetica-Bold").fontSize(16).fillColor("#c07a3a")
         .text(`Tk. ${totalPlatformFee.toLocaleString("en-IN")}`, ML, y);
      y += 30;
      rule(y, C.goldPale, 0.6);
      y += 20;
      ...
      doc.font("Helvetica-Bold").fontSize(8.5).fillColor(C.dust)
         .text("Grand Total (Sales)", ML + 4, y + 7, { width: CW * 0.65 });
      ...
      // Total Platform Fee row
      rule(y, C.rule, 0.4);
      y += 2;
      doc.font("Helvetica").fontSize(8).fillColor(C.dust)
         .text(`Total Platform Fee  (${records.length} orders × Tk. ${PLATFORM_FEE})`, ML + 4, y + 6, { width: CW * 0.7 });
      doc.font("Helvetica-Bold").fontSize(11).fillColor("#c07a3a")
         .text(`Tk. ${totalPlatformFee.toLocaleString("en-IN")}`, ML, y + 4, { width: CW - 4, align: "right" });
      y += 24;
      rule(y, C.rule, 0.5);
      y += 20;
      ...
      ensureSpace(130);
      // Heading
      doc.rect(ML, y, 2, 16).fill(C.goldPale);
      doc.font("Helvetica-Bold").fontSize(9).fillColor(C.ink)
         .text("Authorisation & Signatures", ML + 10, y + 3);
      y += 26;

      // Per-page owner signature boxes + accountant box
      const sigW  = Math.floor((CW - 20 * pages.length) / (pages.length + 1));
      const sigH  = 65;
      const minSigW = 100;

      // If many pages, split into two rows; each box min 100px wide
      const boxW = Math.max(minSigW, Math.floor((CW - 10) / Math.min(pages.length + 1, 4)));
      let sigX   = ML;
      let rowStartY = y;
      let col    = 0;

      const drawSigBox = (title, subtitle) => {
        if (sigX + boxW > ML + CW + 2) {
          // wrap to next row
          y = rowStartY + sigH + 12;
          rowStartY = y;
          sigX = ML;
        ...
        ensureSpace(sigH + 10);
        doc.rect(sigX, y, boxW - 4, sigH).strokeColor(C.rule).lineWidth(0.5).stroke();
        doc.font("Helvetica-Bold").fontSize(6.5).fillColor(C.dust)
           .text(title, sigX + 6, y + 7, { width: boxW - 16 });
        doc.font("Helvetica").fontSize(6).fillColor(C.dust)
           .text(subtitle, sigX + 6, y + 17, { width: boxW - 16 });
        doc.moveTo(sigX + 6, y + sigH - 12).lineTo(sigX + boxW - 10, y + sigH - 12)
           .strokeColor(C.rule).lineWidth(0.4).stroke();
        doc.font("Helvetica").fontSize(5.5).fillColor(C.dust)
           .text("Signature & Date", sigX + 6, y + sigH - 10, { width: boxW - 16 });
        sigX += boxW + 4;
        col++;
      };

      pages.forEach(([pageName]) => {
        drawSigBox("PAGE OWNER", pageName);
      });
      drawSigBox("ACCOUNTANT", "Account Clearance");
    ...
    const fname = `alishan-crafts-sales-${now.toISOString().slice(0,10)}.pdf`;
...
// ── Admin credential verification (reads from .env, never exposes passwords) ──
app.post("/api/admin/login", (req, res) => {
  const { username, password } = req.body;
  if (!username || !password) {
    return res.status(400).json({ success: false, message: "Username and password required." });
  }
  const envKey = `ADMIN_PASSWORD_${username}`;
...
app.listen(PORT, () => {
  console.log(`🚀  Alishan Crafts server running → http://localhost:${PORT}`);
